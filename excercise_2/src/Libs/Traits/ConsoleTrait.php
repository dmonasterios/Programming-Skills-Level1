<?php
namespace Blindma1den\Excercise2\Libs\Traits;
...
trait ConsoleTrait{
  ...
  function promptNumberFromRange($minValue, $maxValue, $title){
    $question = new Question(
      $title,
      "0"
    );
    $question->setValidator(function ($answer) use ($minValue, $maxValue) {
      $answer = intval($answer);
      if (!is_numeric($answer) || $answer < $minValue || $answer > $maxValue) {
        throw new \RuntimeException(
