...
type askParams = {msj?: string, defaultValue?: string}

async function ask({msj, defaultValue}: askParams ): Promise<string> {
  const { response }: { response: string } = await inquirer.prompt({
    name: "response",
    message: msj || "Tell me someting",
    default: defaultValue || "default reponse",
  });
  return response;
}
...
async function date(): Promise<Date> {
  ...
  do {
    ...
    if (!Number.isNaN(date.getTime())) {
      ...
      console.log(
        "Invalid Date. Please introduce a date in format YYYY/MM/DD or MM/DD/YYYY."
      );
    }
  } while (true);
}
